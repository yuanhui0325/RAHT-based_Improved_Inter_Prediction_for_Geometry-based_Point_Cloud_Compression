...
int schro_arith_encode_getbit(SchroArith *arith, uint16_t *probability, int value);
...
static inline int
_schro_arith_encode_getbit(SchroArith *arith, uint16_t *probability, int value)
{
  int bits = arith->getbitlookup[(*probability) >> 8][value];
  return bits;
}
